...
def post(id):
    ...
    if not post:
        flash('Sorry!The is no post was to find!')
...
def create():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(post)
        db.session.commit()
        flash('You are pretty good. Your post has been created!')
...
def edit(id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('You are pretty good! Your post has been updated!', 'success')
        return redirect(url_for('posts.my_posts'))
...
def delete(id):
    post = Post.query.filter_by(id=id).first()
    if not post:
        flash('Sorry!There is no post was to find!')
        return redirect(url_for('posts.my_posts'))
    ...
    db.session.commit()
    flash('"{}" Your post was deleted!'.format(title))
    return redirect(url_for('posts.my_posts'))
